...
local PLUGIN_UPDATE_PATH = "/storage/emulated/0/解说/Plugins/Advance voice assistant by tech for V I develop by Muhammad hanzla/main.lua"
    ...
    local function performUpdate()
        Http.get(SCRIPT_URL, function(code, newContent)
            if code == 200 and newContent then
                local tempPath = PLUGIN_UPDATE_PATH .. ".temp_update"
                local backupPath = PLUGIN_UPDATE_PATH .. ".backup"

                local function restoreFromBackup()
                    if File(backupPath).exists() then
                        os.rename(backupPath, PLUGIN_UPDATE_PATH)
                        return true
                    end
                    return false
                end
                ...
                if f then
                    ...
                    if File(PLUGIN_UPDATE_PATH).exists() then
                        local backupFile = io.open(PLUGIN_UPDATE_PATH, "r")
                        if backupFile then
                    ...
                    local success = pcall(function()
                        os.remove(PLUGIN_UPDATE_PATH)
                        os.rename(tempPath, PLUGIN_UPDATE_PATH)
                    end)
                    ...
                    if success then
                        ...
                        successDialog.setButton2("Later", function()
                            successDialog.dismiss()
                            speak("Update postponed")
                        end)
                        successDialog.show()
