...
local CONSTANTS = {
    ...
    GITHUB_RAW_URL = "https://raw.githubusercontent.com/honeychyt4645ch-bit/Advance-voice-assistant-by-tech-for-V-I/main/",
    DELAYS = {
        SUPER_FAST = 30,
        VERY_FAST = 50,
        FAST = 80,
...
local GITHUB_RAW_URL = CONSTANTS.GITHUB_RAW_URL
        ...
        local success, err = pcall(function()
            ...
            if vibrator and vibrator.hasVibrator() then
                if android.os.Build.VERSION.SDK_INT >= 26 then
                    local effect = VibrationEffect.createOneShot(30, VibrationEffect.DEFAULT_AMPLITUDE)
...
function requestAudioFocus()
    ...
    if audioManager and not isAudioFocusGranted then
        local result = audioManager.requestAudioFocus(nil, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT)
...
function checkUpdate()
    if updateInProgress then 
        speak("Update check in progress")
        return 
    end
    ...
    local success, err = pcall(function()
        Http.get(VERSION_URL, function(code, onlineVersion)
            if code == 200 and onlineVersion then
                ...
                speak("Cannot check for updates")
            end
        end)
    end)
    if not success then
...
function showUpdateDialog(onlineVersion)
    ...
    updateDlg.setButton("Update Now", function()
        ...
        downloadAndInstallUpdate(onlineVersion)
    end)
    updateDlg.setButton2("Later", function()
        updateDlg.dismiss()
        speak("Update postponed")
    end)
    updateDlg.show()
end

function downloadAndInstallUpdate(onlineVersion)
    updateInProgress = true
    local function performUpdate()
        Http.get(SCRIPT_URL, function(code, newContent)
            if code == 200 and newContent then
                -- Get current script path
                local scriptPath = service.getLuaDir and service.getLuaDir() or "/storage/emulated/0/解说/Plugins/Advance voice assistant by tech for V I develop by Muhammad hanzla/main.lua"
                if service.getLuaDir then
                    scriptPath = service.getLuaDir() .. "/main.lua"
                end
                ...
                if f then
                    f:write(newContent)
                    f:close()
                    if File(scriptPath).exists() then
                        local backupFile = io.open(scriptPath, "r")
                        if backupFile then
                    ...
                    if success then
                        ...
                        successDialog.setMessage("Update installed successfully! Version " .. CURRENT_VERSION .. " → " .. onlineVersion .. "\n\nPlease restart the plugin to apply changes.")
                        successDialog.setButton("Restart Now", function()
                            successDialog.dismiss()
                            speak("Update successful. Restarting...")
                        ...
                        if restored then
                            errorDialog.setTitle("⚠️ Update Failed")
                    ...
                    local errorDialog = LuaDialog(service)
                    errorDialog.setTitle("⚠️ Update Failed")
                    errorDialog.setMessage("Cannot write temporary file: " .. tostring(openErr))
                    errorDialog.setButton("OK", function()
                        errorDialog.dismiss()
                        speak("Update failed, cannot write file.")
                    end)
                    errorDialog.show()
                end
            else
                local errorDialog = LuaDialog(service)
                errorDialog.setTitle("⚠️ Update Failed")
                errorDialog.setMessage("Cannot download new script. HTTP code: " .. tostring(code))
                errorDialog.setButton("OK", function()
                    errorDialog.dismiss()
                    speak("Update failed, download error.")
                end)
                errorDialog.show()
            end
...
function runSystem(input)
    ...
    if vol then
        ...
        if stream ~= -1 then
            am.setStreamVolume(stream, math.floor((vol/100)*am.getStreamMaxVolume(stream)), 1)
